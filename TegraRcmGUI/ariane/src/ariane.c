...
UC_CommandType usb_command_read()
{
    ...
    if (rcm_usb_device_read_ep1_out_sync(usb_read_buff, USB_BUFFER_LENGTH, &bytesTransferred) || !(bytesTransferred > 2))
    //if(usb_device_read_ep1_out(usb_read_buff, USB_BUFFER_LENGTH, &bytesTransferred, true) || !(bytesTransferred > 2))
        return NONE;

    UC_Header* uc = (UC_Header*)usb_read_buff;
    if (uc->signature == COMMAND || uc->signature == EXEC_COMMAND)
    {
        gfx_printf("usb_command_read \n");
        return uc->command;
    }
    return NONE;
}
...
void hos_set_keys(bool allow_sept_reboot)
{
    ...
    sys_pkg1_id = pkg1_identify(pkg1);
    if (!sys_pkg1_id)
    {
    ...
    if (sys_pkg1_id->kb >= KB_FIRMWARE_VERSION_700 && !h_cfg.sept_run)
    {
        ...
        {
            if (!allow_sept_reboot)
            {
                gfx_printf("Need to launch Sept first\n");
                goto out;
            }
            b_cfg.extra_cfg = EXTRA_CFG_NYX_BIS;
            sdmmc_storage_end(&emmc_storage);
            sdram_lp0_save_params(sdram_get_params_patched());
            if (!reboot_to_sept((u8 *)tsec_ctxt.fw, sys_pkg1_id->kb, NULL))
            {
...
void set_fs_info()
{
    ...
    di.cfw_ams = (f_stat("BCT.ini", NULL) == FR_OK)
              || (f_stat("atmosphere/BCT.ini", NULL) == FR_OK)
              || (f_stat("atmosphere/config/BCT.ini", NULL) == FR_OK);
...
void get_fw_version(const char *path, const pkg1_id_t *pkg1_id, char *fw_version, bool *exfat)
{
    ...
    for (u32 i=0; i < ARRAY_SIZE(systemTitles); i++) if (systemTitles[i].kb == pkg1_id->kb)
    {
        ...
        filelist = dirlist(path, systemTitles[i].nca_filename, true, false);
        if (filelist && !strcmp(&filelist[0], systemTitles[i].nca_filename))
        {
            strcpy(fw_version, systemTitles[i].fw_version);

            filelist = dirlist(path, systemTitles[i].nca_filename_exfat, true, false);
...
void set_deviceinfo()
{
    ...
    if (!emu_cfg.enabled || di.emunand_type != FILEBASED) // Don't look up for emu fw is filebased emunand (too slow)
        goto out;
    ...
    emu_pkg1_id = pkg1_identify(pkg1);
    free(pkg1);
    if (!emu_pkg1_id)
    {
    ...
    nx_emmc_gpt_parse(&gpt, &emmc_storage);
    system = nx_emmc_part_find(&gpt, "SYSTEM");
    if (!system)
    {
...
out :    
    sdmmc_storage_end(&emmc_storage);
    gfx_printf("set_deviceinfo() ends in %dms\n", get_tmr_ms() - start_tmr);
}
...
bool get_autorcm_state(bool *state)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            gfx_printf("Error reading BCT %d from BOOT0.\n", i);
...
bool set_autorcm_state(bool autoRCM)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            gfx_printf("Error writing BCT %d from BOOT0.\n", i);
            sdmmc_storage_end(&storage);
            return false;
        }
    }
