...
void on_SizeSDFileCommand()
{
    UC_SDIO* uc = (UC_SDIO*)usb_read_buff;
    u32 res = 0;
    FIL fp;

    printk("Received a SIZE_SD_FILE for path : %s\n", uc->path);
...
out:
    printk("Response for STAT_SD_FILE is : %d\n", res);
    send_response((const void*)&res, sizeof(u32)); // Notify caller
}

void on_ReadSDFileCommand()
{
    UC_SDIO* uc = (UC_SDIO*)usb_read_buff;
    u32 res = 0;
    FIL fp;

    //printk("Received a READ_SD_FILE for path : %s\n", uc->path);
...
{
    printk("on_getDeviceInfoCommand()\n");
    ...
    printk("deviceInfo sent, size = %d / %d, battery %d, fuses %d )\n", bytes, sizeof(UC_DeviceInfo), di.battery_capacity, di.burnt_fuses);
}
    ...
    while(1)
    {
        ...
        switch (usb_command_read())
        {
            case GET_STATUS :
                ...
                break;
            case SIZE_SD_FILE:
                on_SizeSDFileCommand();
                break;
            case READ_SD_FILE:
                on_ReadSDFileCommand();
                break;
            case WRITE_SD_FILE :
